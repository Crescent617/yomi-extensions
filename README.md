...
| skill | [`skills/memory-system/`](skills/memory-system/SKILL.md) | agent 记忆系统 bootstrap：AGENTS.md 记忆块、memory/ 目录 + recall 检索脚本、dream + janitor 自进化 cron |

janitor 不再单独规划——它作为 memory-system 的两个常驻 cron 之一（完整 prompt）收录在该 skill 中。模板机制（`agent` 工具 `template` 参数、ROLE.md 约定）说明在 yomi 内核的工具 desc 与设计文档中，不再需要单独 skill。官方内置模板 planner/verifier/explorer/reviewer 预置在 yomi 内核（`crates/kernel/src/agent_tmpl/`），不在此仓。
